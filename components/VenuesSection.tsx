...
export default function VenuesSection() {
  return (
    <section className="scroll-mt-24 border-t border-white/10 bg-[#090909] py-12 sm:py-20" id="venues">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 sm:mb-12 max-w-2xl">
          <p className="text-xs sm:text-sm uppercase tracking-[0.36em] text-gold">Venues</p>
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {venues.map((venue) => (
            <article key={venue.name} className="rounded-2xl sm:rounded-[32px] border border-white/10 bg-[#111111]/80 p-6 sm:p-8 shadow-soft transition hover:-translate-y-1 hover:border-gold/40">
              <h3 className="text-xl sm:text-2xl font-semibold text-white">{venue.name}</h3>
              <p className="mt-2 sm:mt-3 text-xs sm:text-sm uppercase tracking-[0.28em] text-gold">{venue.capacity}</p>
              <p className="mt-3 sm:mt-4 text-xs sm:text-sm leading-6 sm:leading-7 text-white/70">{venue.description}</p>
              <Link href="/contact" className="mt-6 sm:mt-8 inline-flex rounded-lg sm:rounded-full border border-gold/40 bg-transparent px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm font-semibold uppercase tracking-[0.24em] text-gold transition hover:bg-gold hover:text-ebony">
                Inquire Now
              </Link>
            </article>
          ))}
